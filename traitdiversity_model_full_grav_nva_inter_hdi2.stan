...
//explanatory variables for each component
 ...
 real gNC[res]; //predictor gravNC
 real gNP[res]; //predictor gravNP
 ...
 real gNC2[fis]; //predictor gravNC
 real gNP2[fis]; //predictor gravNP
...
//reserve component
for (i in 1:res){ 
  mu[i] = intercept_reserves_rc[prc[i]] +beta[1]*dd[i]+beta[2]*ds[i]+beta[3]*rh_c[i]+beta[4]*rh_f[i]+beta[5]*rh_b[i]+beta[6]*cm_pc[i]+beta[8]*sa[i]+ beta[9]*si[i]+ beta[10]*ag[i]+ beta[11]*at[i]+ beta[12]*op[i]+ beta[13]*cs[i]+ beta[14]*pg[i]+ beta[15]*gNC[i]+ beta[16]*gNP[i]+ beta[17]*rl[i]+ beta[18]*ps[i]+ beta[19]*hdi[i]+ beta[23]*hdi[i]^2;
}
//fished component
for (i in 1:fis){ 
  mu2[i] = intercept_fished_rc[prc2[i]]+beta[1]*dd2[i]+beta[2]*ds2[i]+beta[3]*rh_c2[i]+beta[4]*rh_f2[i]+beta[5]*rh_b2[i]+beta[6]*cm_pc2[i]+beta[7]*cm_ds[i]+beta[8]*sa2[i]+ beta[11]*at2[i]+ beta[12]*op2[i]+ beta[13]*cs2[i]+ beta[14]*pg2[i]+ beta[21]*gNC2[i]+ beta[22]*gNP2[i]+ beta[17]*rl2[i]+ beta[18]*ps2[i]+ beta[19]*hdi2[i]+ beta[20]*mr[i]+ beta[23]*hdi2[i]^2;
  }
}
model {
 //priors
